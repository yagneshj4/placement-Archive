...
export const ragQuery = async (req, res, next) => {
	try {
		...
		const { answer: mlAnswer, sources, retrieval_ms, llm_ms, total_ms, cached } = mlResponse.data
		...
		// Step 3: Use the LLM/RAG answer generated by the ML service if available, otherwise fallback to rich local formatting
		const answer = mlAnswer || buildRichAnswer(query, sources, expMap)
